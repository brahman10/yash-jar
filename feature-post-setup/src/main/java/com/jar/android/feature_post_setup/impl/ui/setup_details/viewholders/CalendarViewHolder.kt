...
import com.jar.android.feature_post_setup.impl.ui.setup_details.adapter.SetupCalenderDayAdapter
...
internal class CalendarViewHolder(
    private val binding: FeaturePostSetupCellCalenderBinding,
    ...
    private fun setupSavingActionsUI() {
        ...
        binding.clBtn1.setText(calenderViewPageItem?.calendarSavingOperations?.savingOperations?.getOrNull(0)?.title.orEmpty())
        Glide.with(binding.root.context)
            .load(calenderViewPageItem?.calendarSavingOperations?.savingOperations?.getOrNull(0)?.icon)
            .into(object : CustomTarget<Drawable>() {
                override fun onResourceReady(resource: Drawable, transition: Transition<in Drawable>?) {
                    binding.clBtn1.setCompoundDrawablesRelativeWithIntrinsicBounds(resource, null, null, null)
                }
        ...
        binding.clBtn2.setText(calenderViewPageItem?.calendarSavingOperations?.savingOperations?.getOrNull(1)?.title.orEmpty())
        Glide.with(binding.root.context)
            .load(calenderViewPageItem?.calendarSavingOperations?.savingOperations?.getOrNull(1)?.icon)
            .into(object : CustomTarget<Drawable>() {
                override fun onResourceReady(resource: Drawable, transition: Transition<in Drawable>?) {
                    binding.clBtn2.setCompoundDrawablesRelativeWithIntrinsicBounds(resource, null, null, null)
                }
        ...
        binding.clBtn2.setDebounceClickListener {
            calenderViewPageItem?.calendarSavingOperations?.savingOperations?.getOrNull(1)?.let {
                onDSOperationCtaClicked.invoke(it)
            }
        }
    }
    ...
    private fun setLadderingInfo(){
        binding.clLadderContainer.isVisible = calenderViewPageItem?.ladderData != null
        calenderViewPageItem?.ladderData?.let {
