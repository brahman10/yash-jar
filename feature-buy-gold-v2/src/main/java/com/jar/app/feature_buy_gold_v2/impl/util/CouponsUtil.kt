...
fun createBulletList(items: Pair<List<String>, List<String>>,context: Context): Spannable {
    ...
    val drawable = ContextCompat.getDrawable(context, R.drawable.gradient_bullet)
...
fun CouponCode.isCouponExpired(): Boolean {
    val currentSystemAndBEDiff = System.currentTimeMillis() - currentTimestamp.orZero()
    return getExpiryTimeStampInMillis() <= 0
            || currentTimestamp.orZero() <= 0
            || currentSystemAndBEDiff < -60000L //1 minute buffer
}

fun CouponCode.getExpiryTimeStampInMillis() = validityInMillis.orZero() - System.currentTimeMillis()
