...
import com.jar.app.core_analytics.EventKey.Shown_StartNowScreen_Onboarding_Ts
...
import com.jar.app.core_ui.onboarding_stories.OnboardingStoryAdapter
import com.jar.app.databinding.FragmentOnboardingStoryVariant0Binding
...
@AndroidEntryPoint
internal class OnBoardingStoryFragmentVariant0 : BaseFragment<FragmentOnboardingStoryVariant0Binding>() {
    ...
    private var apiCallTimeInit: Long? = null
    ...
    private val storyAdapter = OnboardingStoryAdapter()
    ...
    companion object {
        ...
        fun getInstance(onboardingStoryData: OnboardingStoryData): Fragment {
            val fragment = OnBoardingStoryFragmentVariant0()
            val bundle = bundleOf()
    ...
    override val customBindingInflater: (LayoutInflater, ViewGroup?, Boolean) -> FragmentOnboardingStoryVariant0Binding
        get() = FragmentOnboardingStoryVariant0Binding::inflate
    ...
    private fun setupUI() {
        ...
        onBoardingStories = onboardingStoryData?.stories
        storyType = onboardingStoryData?.storyType
        val stories = mutableListOf<com.jar.app.core_ui.onboarding_stories.data.Stories>()

        if (!onBoardingStories.isNullOrEmpty()) {
            onBoardingStories!!.forEach {
                val story = com.jar.app.core_ui.onboarding_stories.data.Stories(
                    image = it.image,
                    title = it.title
                )
                stories.add(story)
    ...
    private fun setupListeners() {
        binding.btnNext.setOnClickListener {
            ...
            EventBus.getDefault().post(OnboardingStoryImageResourceReadyEvent(timeTaken = currentTime))
            ...
            if (binding.btnNext.getText() == getString(R.string.next))
                moveToNextSlide()
            else {
        ...
        view?.setOnTouchListener { _, _ ->
            binding.tooltipContinue.isVisible = false
            false
        }
                            ...
                            if (diffX > 0) {
                                ...
                                analyticsHandler.postEvent(
                                    ...
                                    mapOf(
                                        ...
                                        BaseConstants.storyNum to (storyNumber + 1).toString(),
                                        BaseConstants.storyType to storyType.toString()
                                    )
                                )
                            } else {
    ...
    private fun toggleUI(position: Int) {
        ...
        if (position == onBoardingStories?.size.orZero() - 1 && lifecycle.currentState.isAtLeast(Lifecycle.State.RESUMED)) {
            binding.btnNext.setText(getString(R.string.start_now))
            binding.btnNext.setDrawableEnd(0)
            analyticsHandler.postEvent(
                EventKey.Shown_startnowcta_Onboardingstories,
                mapOf(
                    EventKey.variants to onboardingStoryViewModel.onboardingStoryFlow.asLiveData().value?.data?.data?.storyType.orEmpty()
                ),
    ...
    fun onOnboardingStoryImageResourceReadyEvent(onboardingStoryImageResourceReadyEvent: OnboardingStoryImageResourceReadyEvent) {
        ...
        if (newOnboardingViewModel.responseCount == 1) {
            if (onboardingStoryImageResourceReadyEvent.isFromCache == null) {
                analyticsHandler.postEvent(
                    Shown_StartNowScreen_Onboarding_Ts, mapOf(
