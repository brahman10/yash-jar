...
class OnboardingStoryV2VH(
    private val binding: CellOnboardingStoryVariant2Binding,
    ...
    fun bindData(data: com.jar.app.feature_onboarding.shared.domain.model.Stories) {
        ...
        data.title?.let { binding.tvHeading.text = it }
        data.textColor?.let {
            val color = Color.parseColor(it)
            binding.tvHeading.setTextColor(color)
            binding.tvDescription.setTextColor(color)
        }
        data.description?.let { binding.tvDescription.text = it}
        Glide.with(itemView.context)
            .load(data.bgImage)
            .listener(requestListener)
            .into(object : CustomTarget<Drawable>() {
                override fun onResourceReady(resource: Drawable, transition: Transition<in Drawable>?) {
                    onResourceReadyVH.invoke((System.currentTimeMillis() - onImageLoadStart.orZero()),position)
                    binding.clBackground.background = resource
                }
