...
import com.jar.app.feature_daily_investment.databinding.FeatureDailySavingsDisableDailySavingsConfirmationDialogBinding
...
@AndroidEntryPoint
internal class PauseOrDisableDailySavingsConfirmationBottomSheet :
    BaseBottomSheetDialogFragment<FeatureDailySavingsDisableDailySavingsConfirmationDialogBinding>() {
    ...
    private val args by navArgs<PauseOrDisableDailySavingsConfirmationBottomSheetArgs>()
    ...
    override val customBindingInflater: (LayoutInflater, ViewGroup?, Boolean) -> FeatureDailySavingsDisableDailySavingsConfirmationDialogBinding
        get() = FeatureDailySavingsDisableDailySavingsConfirmationDialogBinding::inflate

    private var isAutoInvestDisabled = false

    override fun setup() {
        analyticsHandler.postEvent(DailySavingsEventKey.Shown_DisableDailySavingsPopUp)
        setupUI()
        setupListeners()
        observeLiveData()
    }
    ...
    private fun setupUI() {
        binding.lottieView.playLottieWithUrlAndExceptionHandling(
            requireContext(),
            BaseConstants.LottieUrls.SAD_EMOJI
        )
                        ...
                        coreUiApi.openGenericPostActionStatusFragment(
                            ...
                            analyticsHandler.postEvent(
                                DailySavingsEventKey.Shown_StopConfirmation_DSSettings, mapOf(
                                    DailySavingsEventKey.Status to DailySavingsEventKey.disabled
                                )
                            )
                            EventBus.getDefault().post(DisableDailySavingEvent())
                            EventBus.getDefault().post(RefreshDailySavingEvent())
                            popBackStack(
                                R.id.pauseOrDisableDailySavingsConfirmationBottomSheet,
                                true
                            )
                        }
                    },
