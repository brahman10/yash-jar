...
class RepaymentTxnHistoryViewModel constructor(
    ...
    private val _transactionListFlow =
        MutableStateFlow<RestClientResult<ApiResponseWrapper<List<EmiTxnCommonData>?>>>(
            RestClientResult.none()
        )
    val transactionListFlow: CStateFlow<RestClientResult<ApiResponseWrapper<List<EmiTxnCommonData>?>>>
        get() = _transactionListFlow.toCommonStateFlow()

    fun fetchStaticContent(loanId: String) {
        viewModelScope.launch {
            fetchStaticContentUseCase.fetchLendingStaticContent(
                loanId,
                LendingConstants.StaticContentType.REPAYMENT_TRANSACTION_SCREEN
            ).collect {
    ...
    fun fetchTransactionList(loanId: String) {
        viewModelScope.launch {
            fetchEmiTxnHistoryUseCase.getEmiTxnHistory(
                loanId,
                LendingConstants.TransactionType.TRANSACTION
            ).collect {
                _transactionListFlow.emit(it)
            }
        }
    }
}