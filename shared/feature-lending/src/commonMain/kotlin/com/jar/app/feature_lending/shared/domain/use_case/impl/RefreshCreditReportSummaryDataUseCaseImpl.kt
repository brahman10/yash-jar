package com.jar.app.feature_lending.shared.domain.use_case.impl
import com.jar.app.feature_lending.shared.data.repository.LendingRepository
import com.jar.app.feature_lending.shared.domain.use_case.RefreshCreditReportSummaryDataUseCase

internal class RefreshCreditReportSummaryDataUseCaseImpl(
    private val lendingRepository: LendingRepository
) : RefreshCreditReportSummaryDataUseCase {
    override suspend fun refreshCreditReportSummary() = lendingRepository.refreshCreditReportSummary()
}