package com.jar.app.feature_lending.shared.domain.use_case.impl

import com.jar.app.feature_lending.shared.data.repository.LendingRepository
import com.jar.app.feature_lending.shared.domain.use_case.FetchTransactionDetailsUseCase

internal class FetchTransactionDetailsUseCaseImpl constructor(
    private val lendingRepository: LendingRepository
) : FetchTransactionDetailsUseCase {

    override suspend fun getTransactionDetails(paymentId: String) = lendingRepository.getTransactionDetails(paymentId)

}