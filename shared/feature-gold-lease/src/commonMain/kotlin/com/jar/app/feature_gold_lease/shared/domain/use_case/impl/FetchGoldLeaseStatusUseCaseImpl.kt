package com.jar.app.feature_gold_lease.shared.domain.use_case.impl

import com.jar.app.feature_gold_lease.shared.data.repository.GoldLeaseRepository
import com.jar.app.feature_gold_lease.shared.domain.use_case.FetchGoldLeaseStatusUseCase

internal class FetchGoldLeaseStatusUseCaseImpl constructor(
    private val goldLeaseRepository: GoldLeaseRepository
): FetchGoldLeaseStatusUseCase{
    override suspend fun fetchGoldLeaseStatus(leaseId: String) = goldLeaseRepository.fetchGoldLeaseStatus(leaseId = leaseId)
}