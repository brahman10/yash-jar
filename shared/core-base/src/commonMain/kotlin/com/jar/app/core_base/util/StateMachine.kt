...
class StateMachine<STATE : Any, EVENT : Any, SIDE_EFFECT : Any> private constructor(
    ...
    private var stateRef = graph.initialState

    val state: STATE
        get() = stateRef

    fun transition(event: EVENT): Transition<STATE, EVENT, SIDE_EFFECT> {
        val fromState: STATE = stateRef
        val transition: Transition<STATE, EVENT, SIDE_EFFECT> = fromState.getTransition(event)
        if (transition is Transition.Valid<STATE, EVENT, SIDE_EFFECT>) {
